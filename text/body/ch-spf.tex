...
In this section of the paper, we apply our methodology to data from a prestigious scholarship and talent investment program. We first describe the program and, most importantly, its selection process. We then provide an overview of the data we use, focusing on how talent is measured and how we formalize the diversity goals of the program. Finally, we apply the SPF estimate to both test the predictions of our model and show how the SPF can be used to improve selection in various ways. Through these applications, we document evidence that the program selected finalists within the SPF -- consistent with the first prediction of our model -- and that the program selected much closer to the SPF after they were given an SPF estimate to aid in selection of their third cohort. 
...
\begin{figure}[!htb]
    ...
    \begin{threeparttable}
      \caption{Summary Statistics:  Program Applicants}  \label{tab:pooled_demo}
      \label{} 
    ...
    \end{tabular} 
        ...
        \end{adjustbox}
              \begin{notes}
                       Raw data come from surveys completed by applicants. Data are pooled across three application years. 
                \end{notes}
    \end{table} 
    ...
    \begin{table}[!htbp] \centering 
     \begin{adjustbox}{width=\textwidth, totalheight=\textheight-2\baselineskip,keepaspectratio}
    \begin{threeparttable}
      \caption{Summary Statistics: Project Reviewers}\label{tab:pooled_rev_demo}
      \label{} 
    \begin{tabular}{@{\extracolsep{5pt}}lccccc} 
    \\[-1.8ex]\hline 
    \hline \\[-1.8ex] 
