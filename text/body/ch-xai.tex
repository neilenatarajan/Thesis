...
\section{Background}
% 4.3.1 – (Overly?) Brief History of XAI This section presents itself as a brief history of XAI, but the development of explainable and interpretable algorithms significantly predates methods like LIME. It may be useful to touch on the early origins of machine learning and contrast them with approaches such as statistical modelling—where a statistician selects a model with a known structure—or expert systems, in which human reasoning is explicitly encoded into rules and justifications.

\subsection{A Brief History of Explainable AI}\label{ssec:history}
%How far and by whom? Two foundational issues appear to be underexplored. First, what does it actually mean for a model to be “interpretable”? Is it about understanding the system itself, the underlying process or distribution, or simply being able to reproduce or control the system’s outputs? Should interpretability be intuitive or technically precise? Does it refer to understanding the true distribution or just the modelled approximation? Second, interpretability is not universal—it can depend on who the audience is. A system might be interpretable to one group but not another. This raises important questions: who is the intended explainee, and how does their background affect the success or value of an explanation?
...
% Interestingly, you opt for the terms “intrinsic” vs “post-hoc” instead of the more commonly used distinctions such as “explainable” vs “interpretable” or “black box” vs “glass box.” A brief explanation of this choice could help orient the reader.
...
% Figure 4.1 – Slightly hard to interpret. If indeed this is a taxonomy, could this be more simply conveyed using a tree?
...
% ⭐️ 4.x – Automation bias as a baseline – A well-documented source of “unwarranted trust” of algorithmic/AI systems is simply automation bias – the simple tendency of people to believe algorithmic systems (because they are “complex and smart” among other reasons). Currently the chapter does not discuss automation bias which seems like it could be seen as a notable omission because it is such a common phenomenon. Please rectify this.  It would be particularly good if one could ascertain to what extent are the results explained by / attributable to simple automation bias? 
...
% “In the case of specific instances of model outputs, though, the question of trustworthiness is more straightforward: when a system outputs a correct (or an incorrect) result, that result is trustworthy (or untrustworthy).” Yes, a bit easier, but not entirely simple, as ‘correctness’ can even be difficult (consider a regression problem) but even say, a binary classification problem can be correct but perhaps discriminatory, which could be deemed as not trustworthy.
...
% ⭐️ Section 4.4.2.5: “We code the participant’s estimate as a binary yhuman variable:  … yhuman := How much money does this person make”. It’s not clear how this is binary. Maybe estimating whether someone areas over a particular salary range? (That does seem familiar for the Adult dataset). A small clarification would be helpful
...
% 4.4.4 – “blindly increasing trust” – It might be useful to critically interrogate this statement. Do we know what is going on here?  Are people believing the algorithm because they have no reason not to?  Or because it looks like an authoritative / complex explanation?  Or is a kind of confirmation bias, such as – might SHAP trigger confirmation bias by showing features in a way that draws people’s attention to aspects they believe would lead to a particular outcome?
...
% “Practitioners desired additional modes of interaction with our explanatory materials, and points" – what sort and what materials?
...
% Great hypothesis. Are there other possible explanations for the observation?
