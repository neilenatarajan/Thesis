...
\subsection{Terms}
We introduce a number of key domain-specific terms; they appear throughout the thesis (though they are sometimes again spelled out for clarity). We define them here – note that our definitions may differ from those in other works.
...
VSD is yet a third family of methods within HCI engaging participants. In this case, though, particular values of participants are elicited, and used as a guide for design. Research outputs are standardly designs and design recommendations driven by careful analysis of user values.
...
\begin{itemize}
    \item A list of decision points facing scholarship programmes uncovered through longitudinal HCI research with Rise and Ellison Scholars (Chapter \ref{ch:context}).
    \item Quantitative findings indicating that the problem of explanation-induced unwarranted trust extends to generic post-hoc justifications, but that such criticism only applies in-process and qualitative findings that post-hoc explanations, properly presented, can make useful ex-post DSTs (Chapter \ref{ch:xai}).
